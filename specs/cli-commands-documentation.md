...
## Overview
The SDK docs promise CLI tooling (`infracost plugin validate`, `infracost plugin add`, and fixture flags) that does not exist. Docs must describe only the plugin-management commands the CLI actually ships.
...
- Remove all references to `infracost plugin validate`, `infracost plugin add`, `--fixture`/`--fixtures`, and the itemized fictional conformance command output from the READMEs, SPECs, and example Makefiles.
...
- [ ] Command examples do not show unsupported flags.

## Edge Cases
- `plugin update` intentionally fails when a custom plugin dir is set — worth stating so authors don't file bugs.
- Future-work references must not imply a release date or that an issue identifier is itself a usable command.
